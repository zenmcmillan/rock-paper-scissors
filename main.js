// query selectors

var classicButton = document.querySelector('.classic-button');
var classicAndHardContainers = document.querySelector('.classic-and-hard');
var hardButton = document.querySelector('.hard-button');
var changeGameButton = document.querySelector('.change-game-button');
var subtitle = document.querySelector('.subtitle');
var personalGameInfo = document.querySelector('.personal-game-info');
var computerGameInfo = document.querySelector('.computer-game-info');

// global variables

var game = {}

// event listeners

classicButton.addEventListener('click', function() {
  goToGame();
  createGameFunctionality(game);
  computerTakingItsTurn(game, ['classicGameBoard']);
});

hardButton.addEventListener('click',function() {
  goToGame();
  createGameFunctionality(game);
  computerTakingItsTurn(game, ['hardGameBoard']);
  
});

changeGameButton.addEventListener('click', function() {
 goBackToHomePage()
});

window.addEventListener('load', function() {
  createGame() //data model
  renderPlayerData() //DOM
 
});

// event handlers

function assignPlayingPieces(game) {
 
}

function createGameFunctionality(game) {
  var player1 = game.player1.chosenPiece
  var player2 = game.player2.chosenPiece
  
   if (player1 === 'rock' && player2 === 'scissors' || player2 === 'lizard') {
    game.player1.wins += 1
    game.player1.wonThisRound = true
   }
   else if (player2 === 'rock' && player1 === 'scissors' || player1 === 'lizard') {
     game.player2.wins += 1
     game.player2.wonThisRound = true
   }
   else if (player1 === 'rock' && player2 === 'rock') {
     game.draw = true
   }
   else if (player1 === 'scissors' && player2 === 'paper' || player2 === 'lizard') {
     game.player1.wins += 1
     game.player1.wonThisRound = true
   }
   else if (player2 === 'scissors' && player1 === 'paper' || player1 === 'lizard') {
     game.player2.wins += 1
     game.player2.wonThisRound = true
  }
  else if (player1 === 'scissors' && player2 === 'scissors') {
     game.draw = true
  }
  else if (player1 === 'paper' && player2 === 'rock' || player2 === 'alien') {
     game.player1.wins += 1
     game.player1.wonThisRound = true
   }
   else if (player2 === 'paper' && player1 === 'rock' || player1 === 'alien') {
     game.player2.wins += 1
     game.player2.wonThisRound = true
  }
  else if (player1 === 'paper' && player2 === 'paper') {
    game.draw = true
  }
  else if (player1 === 'lizard' && player2 === 'paper' || player2 === 'alien') {
    game.player1.wins += 1
    game.player1.wonThisRound = true
  }
  else if (player2 === 'lizard' && player1 === 'paper' || player1 === 'alien') {
    game.player2.wins += 1
    game.player2.wonThisRound = true
  }
  else if (player1 === 'alien' && player2 === 'scissors' || player2 === 'rock') {
    game.player2.wins += 1
    game.player2.wonThisRound = true
  }
  else if (player2 === 'alien' && player1 === 'scissors' || player1 === 'rock') {
    game.player1.wins += 1
    game.player1.wonThisRound = true
  }
  console.log(game.player1);
  console.log(game.player2);
}

function computerTakingItsTurn(game, [array]) {
 var gameChoice = game[array]
 var piece = gameChoice[getRandomIndex(gameChoice)]
 return console.log(piece)
}

function getRandomIndex(array) {
  return Math.floor(Math.random() * array.length)
}

function renderPlayerData() {
  
  personalGameInfo.innerHTML = '';

  personalGameInfo.innerHTML += 
  `<p class="icons">${game.player1.token}</p>
   <p class="player">${game.player1.name}</p>
   <p> Wins:<span> ${game.player1.wins}</span></p>`

   computerGameInfo.innerHTML = '';

   computerGameInfo.innerHTML += 
   `<p class="icons">${game.player2.token}</p>
   <p class="player">${game.player2.name}</p>
   <p> Wins:<span> ${game.player2.wins}</span></p>`
};

function createGame() {
 var player1 = createPlayer('You', '🙂');
 var player2 = createPlayer('Computer', '💻');
 game = {player1, player2, classicGameBoard: ['rock', 'paper', 'scissors'], 
  hardGameBoard: ['rock', 'paper', 'scissors', 'alien', 'lizard'], draw: false}
 console.log(game)
}

function createPlayer(personOrComputer, token) {
  return {name: personOrComputer, token: token,  wins: 0, wonThisRound: false, chosenPiece: null}
};

function goToGame() {
  classicAndHardContainers.classList.add('hidden');
  changeGameButton.classList.remove('hidden');
  subtitle.innerText = 'Choose your fighter!';
};

function goBackToHomePage() {
  classicAndHardContainers.classList.remove('hidden');
  changeGameButton.classList.add('hidden');
  subtitle.innerText = 'Choose your game!';
};

